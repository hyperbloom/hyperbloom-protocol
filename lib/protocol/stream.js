...
function Stream(options) {
  ...
  this.feed = this.options.discoveryKey || this._hash(this.feedKey);
  ...
  this.id = this.options.id || this._randomBytes(ID_SIZE);
  assert.equal(this.id.length, ID_SIZE,
  ...
  if (this.options.preparse)
    this._onOpen(this.options.preparse.open, this.options.preparse.extra);
  else
    this.once('open', (open, extra) => this._onOpen(open, extra));
}
...
Stream.prototype._onOpen = function _onOpen(open, extra) {
  ...
  this._resumeParsing(extra);
};

Stream.prototype._onHandshake = function _onHandshake(body) {
  this._hyperchain.verify(body.chain, this._nonceHash, body.signature);
