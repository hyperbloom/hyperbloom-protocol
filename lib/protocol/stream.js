...
const DEFAULT_SHUTDOWN_TIMEOUT = 1000;
...
Stream.prototype.shutdown = function shutdown() {
  if (this._destroyed || this._shutdown)
    return;

  this._shutdown = true;

  debug('[%s] shutdown', this._debugId);
  this._shutdownTimer = setTimeout(() => {
    debug('[%s] shutdown timeout', this._debugId);
    this.destroy();
